...
def schoolbook_64x13(r_mem, a_mem, b_mem, r_off=0, a_off=0, b_off=0, additive=False):
    ...
    for i in range(13):
        ...
        for j in range(min(i+1, 7)):
            if i - j < 7:
                if first:
                    p("vpmullw %ymm{}, %ymm{}, %ymm{}".format(a_regs[j], b_regs[i-j], 14))
                    first = False
                else:
                    p("vpmullw %ymm{}, %ymm{}, %ymm{}".format(a_regs[j], b_regs[i-j], 15))
    ...
    for i in range(6):
        p("vmovdqa {}({}), %ymm{}".format(32*(7+i + b_off), b_mem, b_regs[i]))
        if additive:
            p("vpaddw {}({}), %ymm{}, %ymm{}".format(32*(7+i + b_off + 13), b_mem, b_regs[i], b_regs[i]))
    ...
    for i in range(6):
        p("vmovdqa {}({}), %ymm{}".format(32*(7+i + a_off), a_mem, a_regs[i]))
        if additive:
            p("vpaddw {}({}), %ymm{}, %ymm{}".format(32*(7+i + a_off + 13), a_mem, a_regs[i], a_regs[i]))
    ...
    for i in range(6):
        p("vmovdqa {}({}), %ymm{}".format(32*(i + b_off), b_mem, b_regs[i]))
        if additive:
            p("vpaddw {}({}), %ymm{}, %ymm{}".format(32*(i + b_off + 13), b_mem, b_regs[i], b_regs[i]))
